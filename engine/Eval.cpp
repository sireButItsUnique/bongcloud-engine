...
double Eval::finishCaptures(Board *board, double eval, double alpha, double beta, int &evaluated) {
	...
	vector<pair<double, Board *>> moves;
	...
	if (board->turn) {
		...
		for (int i = 0; i < moves.size(); i++) {
			...
			// exit immediately if king dead
			if (moves[i].first == DOUBLE_INF || moves[i].first == -1 * DOUBLE_INF) {
				eval = min(eval, moves[i].first);
			} else {
...
double Eval::getBoardEvalRec(Board *board, int ply, double alpha, double beta, int &evaluated) {
	...
	// set up board + eval
	board->genMoves();
	double eval = (board->turn ? 1 : -1) * DOUBLE_INF;
